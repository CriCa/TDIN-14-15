...
namespace BookEditor
{
    public class OrderTable : DatabaseTable
    {
        ...
        {
            return String.Format("create table {0} ({1} INTEGER PRIMARY KEY AUTOINCREMENT, {2} INTEGER NOT NULL, {3} INTEGER NOT NULL, {4} INTEGER NOT NULL DEFAULT 0, {5} INTEGER NOT NULL DEFAULT 0, {6} TEXT, {7} TEXT, {8} REAL NOT NULL DEFAULT 0, FOREIGN KEY({2}) REFERENCES users(id), FOREIGN KEY({3}) REFERENCES books(id));", getTableName(), KEY_ID, KEY_CLIENT_ID, KEY_BOOK_ID, KEY_QUANTITY, KEY_STATE, KEY_DATE, KEY_STATE_DATE, KEY_PRICE);
        }
    }
}
